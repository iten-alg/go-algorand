...
type constBlock struct {
	...
	label string // The string for the label following the block

	soloLabel bool // True iff label was on its own line

	blockSourceLine int // Sourceline of the block

	labelSourceLine int // Sourceline of the label which could be more than one apart from blockSourceLine due to comments
}
...
type constBlockReference struct {
	blockIndex int // index intc or bytec op is trying to get from const block

	// position of intc/bytec opcode
	position int
}
...
type OpStream struct {
	...
	unassembledConstBlocks []constBlock          // Delays assembly of const blocks until we know if they are terminating or not
	hasMultIntcBlocks      bool                  // int pseudo-op becomes pushint if multiple intc blocks
	beginsWithIntcBlock    bool                  // set to true if there is an intcblock op before all other ops except const block ops
	intcBlockRefs          []constBlockReference // lists explicit attempts to index into intcblock so length checking can be done later
	maxIntcBlockLength     int                   // Keeps track of largest intcblock created including potentially one generated by int pseudo-op
	...
	beginsWithBytecBlock bool                  // set to true if there is a bytecblock op before all other ops except const block ops
	bytecBlockRefs       []constBlockReference //lists explicit attempts to index into bytecblock so length checking can be done later
	seenNonConstBlock    bool                  // set to true as soon as we see a spec that is not intcblock or bytecblock
	maxBytecBlockLength  int                   // Keeps track of largest bytecblock created including potentially one generated by byte pseudo-ops
	...
	atEndProgram bool // Set to true if we are done reading all of the lines, used to handle terminating const blocks
}
...
func (ops *OpStream) Intc(constIndex uint) {
	if constIndex < 4 {
		ops.pending.WriteByte(0x22 + byte(constIndex)) // intc_
	} else {
		if constIndex > 0xff {
	...
	// We can't easily know what the value is or the index even due to the optimizations later done
	ops.trace("intc")
}

// Uint writes opcodes for loading a uint literal
func (ops *OpStream) Uint(val uint64) {
	if ops.hasMultIntcBlocks || (ops.hasIntcBlock && !ops.beginsWithIntcBlock) {
...
func (ops *OpStream) Bytec(constIndex uint) {
	if constIndex < 4 {
		ops.pending.WriteByte(0x28 + byte(constIndex)) // bytec_
	} else {
		if constIndex > 0xff {
...
func (ops *OpStream) ByteLiteral(val []byte) {
	if ops.hasMultBytecBlocks || (ops.hasBytecBlock && !ops.beginsWithBytecBlock) {
		// If multiple bytecblocks were created, we can't know which one we'll be in, we just treat this like pushbytes
		// Same thing if we only have a single bytecblock but it was not at the beginning (can't know if the bytecblock's been initialized yet or not)
...
func asmIntC(ops *OpStream, spec *OpSpec, args []string) error {
	...
	if constIndex <= 0xff {
		// If it's over 0xff, it will error in Intc anyway, so we don't need to track it
		ops.intcBlockRefs = append(ops.intcBlockRefs, constBlockReference{blockIndex: int(constIndex), position: ops.pending.Len()})
	}
...
func asmByteC(ops *OpStream, spec *OpSpec, args []string) error {
	...
	if constIndex <= 0xff {
		// If it's over 0xff, it will error in Bytec anyway, so we don't need to track it
		ops.bytecBlockRefs = append(ops.bytecBlockRefs, constBlockReference{blockIndex: int(constIndex), position: ops.pending.Len()})
	}
...
func asmIntCBlock(ops *OpStream, spec *OpSpec, args []string) error {
	if !ops.atEndProgram {
		if ops.hasIntcBlock {
			if ops.maxIntcBlockLength < len(args) {
				ops.maxIntcBlockLength = len(args)
			}
		...
		for i := len(ops.intcRefs) - 1; i >= 0; i-- {
			...
			if err != nil {
				ops.error(err)
				// TODO: Should we keep going here or should we return?
			}
	...
	if !ops.seenNonConstBlock && !ops.atEndProgram {
		ops.beginsWithIntcBlock = true
	}
	return nil
}
...
func asmByteCBlock(ops *OpStream, spec *OpSpec, args []string) error {
	...
	if !ops.seenNonConstBlock {
		ops.beginsWithBytecBlock = true
	}
	if ops.hasBytecBlock {
		if ops.maxBytecBlockLength < len(bvals) {
			ops.maxBytecBlockLength = len(bvals)
		}
...
func (ops *OpStream) assemble(text string) error {
	...
	for scanner.Scan() {
		...
		if opstring[len(opstring)-1] == ':' {
			if ops.unassembledConstBlocks == nil {
				...
				ops.unassembledConstBlocks[len(ops.unassembledConstBlocks)-1].label = opstring[:len(opstring)-1]
				fields = fields[1:]
				if len(fields) == 0 {
					ops.unassembledConstBlocks[len(ops.unassembledConstBlocks)-1].soloLabel = true
					continue
				}
				opstring = fields[0]
			}
		...
		if ok {
			...
			if spec.Name == "intcblock" || spec.Name == "bytecblock" {
				ops.unassembledConstBlocks = append(ops.unassembledConstBlocks, constBlock{spec: &opsByOpcode[ops.Version][spec.Opcode], fields: fields})
			} else {
				if ops.unassembledConstBlocks != nil {
					saved := ops.sourceLine
					for _, block := range ops.unassembledConstBlocks {
						ops.sourceLine = block.blockSourceLine
						block.spec.asm(ops, block.spec, block.fields[1:])
						if len(block.label) > 0 {
							ops.sourceLine = block.labelSourceLine
							ops.createLabel(block.label)
							if !block.soloLabel {
								ops.trace("%3d: label only\n", ops.sourceLine)
							}
						}
					...
					ops.unassembledConstBlocks = nil
				}
				ops.seenNonConstBlock = true
	...
	if !ops.hasIntcBlock {
		ops.maxIntcBlockLength = len(ops.intc)
	}
	if !ops.hasBytecBlock {
		ops.maxBytecBlockLength = len(ops.bytec)
	}
	tempSourceLine := ops.sourceLine
	for _, ref := range ops.intcBlockRefs {
		if ref.blockIndex >= ops.maxIntcBlockLength {
			// Go back and error on any out of bounds intc calls now that we have more info about the intcblocks
			ops.sourceLine = ops.OffsetToLine[ref.position] + 1
			ops.errorf("intc %d is not defined", ref.blockIndex)
		}
	}
	for _, ref := range ops.bytecBlockRefs {
		if ref.blockIndex >= ops.maxBytecBlockLength {
			ops.sourceLine = ops.OffsetToLine[ref.position] + 1
			ops.errorf("bytec %d is not defined", ref.blockIndex)
		}
	}
	ops.sourceLine = tempSourceLine
...
type disassembleState struct {
	...
	hasIntcBlock           bool
	hasBytecBlock          bool
	hasMultipleIntcBlocks  bool
	hasMultipleBytecBlocks bool
	beginsWithIntcBlock    bool
	beginsWithBytecBlock   bool
	seenNonConstBlock      bool
}
...
func disassemble(dis *disassembleState, spec *OpSpec) (string, error) {
	...
	for _, imm := range spec.OpDetails.Immediates {
		...
		case immByte:
			...
			if spec.Name == "intc" && int(b) < len(dis.intc) && !dis.hasMultipleIntcBlocks {
				// Even if the program does not begin with an intcblock, we will still reference into it b/c otherwise it's an error assuming only one intcblock
				out += fmt.Sprintf(" // %d", dis.intc[b])
			}
			if spec.Name == "bytec" && int(b) < len(dis.bytec) && !dis.hasMultipleBytecBlocks {
		...
		case immInts:
			...
			if dis.hasIntcBlock {
				dis.hasMultipleIntcBlocks = true
			}
			if !dis.seenNonConstBlock {
				dis.beginsWithIntcBlock = true
			}
			dis.hasIntcBlock = true
			dis.intc = intc
			for i, iv := range intc {
				if i != 0 {
					out += " "
		...
		case immBytess:
			...
			if dis.hasBytecBlock {
				dis.hasMultipleBytecBlocks = true
			}
			if !dis.seenNonConstBlock {
				dis.beginsWithBytecBlock = true
			}
			dis.hasBytecBlock = true
			dis.bytec = bytec
	...
	if strings.HasPrefix(spec.Name, "intc_") {
		...
		if int(b) < len(dis.intc) && !dis.hasMultipleIntcBlocks {
			out += fmt.Sprintf(" // %d", dis.intc[b])
		}
	}
	if strings.HasPrefix(spec.Name, "bytec_") {
		b := spec.Name[len(spec.Name)-1] - byte('0')
		if int(b) < len(dis.bytec) && !dis.hasMultipleBytecBlocks {
			out += fmt.Sprintf(" // %s", guessByteFormat(dis.bytec[b]))
		}
	}
	dis.nextpc = pc
...
func disassembleInstrumented(program []byte, labels map[int]string, hasMultipleIntcBlocks bool, hasMultipleBytecBlocks bool, rerunning bool, intc []uint64, bytec [][]byte) (text string, ds disInfo, err error) {
	out := strings.Builder{}
	dis := disassembleState{program: program, out: &out, pendingLabels: labels, hasMultipleIntcBlocks: hasMultipleIntcBlocks, hasMultipleBytecBlocks: hasMultipleBytecBlocks, intc: intc, bytec: bytec}
	...
	text = out.String()
	if !rerunning {
		if dis.rerun || (dis.hasIntcBlock && !dis.beginsWithIntcBlock) || (dis.hasBytecBlock && !dis.beginsWithBytecBlock) || dis.hasMultipleIntcBlocks || dis.hasMultipleBytecBlocks {
			return disassembleInstrumented(program, dis.pendingLabels, dis.hasMultipleIntcBlocks, dis.hasMultipleBytecBlocks, true, dis.intc, dis.bytec)
		}
	}
	return
}
...
func Disassemble(program []byte) (text string, err error) {
	text, _, err = disassembleInstrumented(program, nil, false, false, false, nil, nil)
	return
}

// HasStatefulOps checks if the program has stateful opcodes
func HasStatefulOps(program []byte) (bool, error) {
	_, ds, err := disassembleInstrumented(program, nil, false, false, false, nil, nil)
